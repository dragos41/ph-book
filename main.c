...
void namefun(){
	...
	double phone;
	char gender[8];
	fptr=fopen("drag.txt","ab+");//ab+ gives us the ability of writing the function and add the second data in the existing one...
	if(fptr==NULL){
	...
	char ch;
	gotoxy(31,4);
	printf("Do you wanna add more data.Press w for that:");
	Sleep(1000);
	fflush(stdin);
	while((ch=getch())=='y'){
		menu();
...
void searchfun(){
	...
	while(fscanf(fptr,"%s %s %s %s %lf\n",name,address,gender,gmail,&phone)!=EOF){
		...
		Sleep(1000);
			gotoxy(18,12);
			printf("Enter w for menu option.");
	while(getch()=='y'){
		menu();
	}
	}
	...
	if(flag==0){
		...
		gotoxy(39,6);
		printf("Enter a to enter file again or double w key to open menu section:");
		if(getch()=='a'){
			system("cls");
			searchfun();
		}
...
void listfun(){
	...
	while(fscanf(fptr,"%s %s %s %s %lf",name,address,gender,gmail,&phone)!=EOF){
		...
		Sleep(1000);
		printf("Enter w for menu section:");
		while(getch()=='y'){
			menu();
		}
			fclose(fptr);
...
void modifyfun(){
	...
	fclose(fptr);
	fclose(fptr1);
	fptr=fopen("drag.txt","w");
	fclose(fptr);
	fptr=fopen("drag.txt","a");
	fptr1=fopen("drag1.txt","r");
	...
	fclose(fptr);
	fclose(fptr1);
	fptr1=fopen("drag1.txt","w");
	fclose(fptr1);
	printf("\n\nPress w for menu option.");
	fflush(stdin);
	if(getch()=='y'){
		menu();
	}
...
void exitfun(){
	...
	gotoxy(31,6);
	printf("\xDB Andrei Mihai.");
	gotoxy(31,8);
	printf("\xDB Nelus Costin.");
	gotoxy(31,10);
	printf("\xDB Ion Ionel.");
	gotoxy(31,12);
	printf("\xDB Craciun Dragos.");
}
void password(void){
